...
public abstract class BaseSetUpActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mGestureDetector = new GestureDetector(this, new GestureDetector.SimpleOnGestureListener() {
            /**
             *
             * @param e1 手指第一次接触屏幕的事件
             * @param e2 手指离开屏幕一瞬间的事件
             * @param velocityX 水平方向上的速度 单位pix/s
             * @param velocityY 竖直方向上的速度 单位pix/s
             * @return
             */
            @Override
            public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
                if (Math.abs(velocityX) < 200) {
                    ...
                    return true;
                } else if ((e2.getRawX() - e1.getRawX()) > 200) {//从左向右滑动,显示上一个界面
                    showPre();
                    overridePendingTransition(R.anim.pre_in, R.anim.pre_out);
                    return true;
                } else if ((e1.getRawX() - e2.getRawX()) > 200) {//从右向左滑动,显示下一个界面
                    showNext();
